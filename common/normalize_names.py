...
_CORP_SUFFIX_RE = re.compile(r"\b(inc|incorporated|ltd|limited|corp|corporation|llp|lp|the|co)\b")


def org_key(name: str) -> str:
    """Match key for organization-name merging -- strips accents, case,
    punctuation, and common corporate suffixes. Moved here 2026-08-05
    (was build_knowledge_graph.py's local org_key()) so TorontoSpends'
    entity-resolution can share the same merge logic Follow the Money's
    knowledge graph already uses and has tested."""
    key = strip_accents_for_matching(normalize_whitespace(name or "")).lower()
    key = re.sub(r"[^a-z0-9 ]", " ", key)
    key = _CORP_SUFFIX_RE.sub(" ", key)
